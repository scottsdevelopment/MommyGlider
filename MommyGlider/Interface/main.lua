...
-- Track last movement and last drink time
global_lastMoveTime = 0
global_lastDrinkTime = 0
...
-- Helper function to check if we should attempt to drink
local function canDrink()
    local currentTime = GetTime()
    print(currentTime - global_lastMoveTime)
    print(currentTime - global_lastDrinkTime)
    return (currentTime - global_lastMoveTime) > 5 and (currentTime - global_lastDrinkTime) > 5
end

-- Track movement and drink attempts
local function onMove()
    global_lastMoveTime = GetTime()
end

local function onDrink()
    global_lastDrinkTime = GetTime()
end
...
function CoreRotations:addSpellRotation(spell, options)
    self:addRotation({
        condition = function() 
          ...
          --  print(spell)
           ...
           -- print(spell)
            ...
            if options.castingCheck and UnitCastingInfo("player") then return false end
            if options.condition and not options.condition() then return false end
            -- print(spell)
        ...
        action = function(target)
            if options.action then return options.action() end
...
function CoreRotations:initialize()
    ...
    CoreRotations:addSpellRotation("Drink", {
        unit = "player",
        maxPower = 60,
        notInCombat = true,
        buff = "Drink",
   ...
   self:addRotation({
    ...
    self:addSpellRotation("Shadow Word: Pain", {
        ...
        maxHealthThreshold = 20,          -- Avoid casting on targets with ≤ 30% health
        minPower = 10,                 -- Avoid casting if mana is below 20%
    })
    
    -- Smite when not on cooldown
    self:addSpellRotation("Smite", {
        unit = "target",
        maxHealthThreshold = 15,          -- Avoid casting on targets with ≤ 30% health
        minPower = 10,
    })
...
function CoreRotations:execute()
    -- Register movement tracking
    if IsPlayerMoving() then onMove() end
